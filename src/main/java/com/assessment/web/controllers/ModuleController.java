...
public class ModuleController {
	...
	public ModelAndView modules(@RequestParam(name= "page", required = false) Integer pageNumber, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		...
		Page<Module> modules = moduleService.getModules(user.getCompanyId(), PageRequest.of(pageNumber, 15));
	...
	public ModelAndView addModule( @RequestParam(name= "moduleId", required = false) Long moduleId,  HttpServletRequest request, HttpServletResponse response) throws Exception {
		Module module = null;
			if(moduleId != null){
				module = moduleService.getModuleById(moduleId);
				request.getSession().setAttribute("mname", module.getModuleName());
...
//					}
	...
	public ModelAndView addModuleitem(@ModelAttribute("module") Module module, @RequestParam(name= "moduleItemId", required = false) Long moduleItemId, @RequestParam(name= "mname", required = true) String mname, HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public ModelAndView removemoduleitem(@ModelAttribute("module") Module module, @RequestParam(name= "moduleItemId", required = true) Long moduleItemId, @RequestParam(name= "moduleName", required = true) String moduleName,HttpServletRequest request, HttpServletResponse response){
		Set<ModuleItem> items = (Set<ModuleItem>) request.getSession().getAttribute("moduleItems");
			for(ModuleItem item : items){
				if(item.getId().equals(moduleItemId)){
					items.remove(item);
	...
	public ModelAndView saveModuleitem(@ModelAttribute("moduleItem") ModuleItem moduleItem, HttpServletRequest request, HttpServletResponse response) throws Exception {
		User user = (User) request.getSession().getAttribute("user");
		Set<ModuleItem> items = (Set<ModuleItem>) request.getSession().getAttribute("moduleItems");
		Module module = (Module)request.getSession().getAttribute("module");
	...
	public ModelAndView saveModule(@ModelAttribute("module") Module module, HttpServletRequest request, HttpServletResponse response , ModelMap modelMap) throws Exception {
		User user = (User) request.getSession().getAttribute("user");
		Set<ModuleItem> items = (Set<ModuleItem>) request.getSession().getAttribute("moduleItems");
		module.setItems(items);
		...
		try {
			Long id = module.getId();
				if(module.getModuleName() == null){
					String name = (String)request.getSession().getAttribute("mname");
...
//			}
			...
			if(e.getMessage().equalsIgnoreCase("MODULE_NAME_EXISTING")){
				...
				mav.addObject("module", module);
				mav.addObject("licenses", licenses);
				mav.addObject("message", "Module name already exists. Use a different name");// later put it as label
				mav.addObject("msgtype", "Information");
				return mav;
			}
		}
