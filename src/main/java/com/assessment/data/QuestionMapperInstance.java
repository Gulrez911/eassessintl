...
public class QuestionMapperInstance extends Base{
	...
	public Boolean getCorrect() {
		String type = getQuestionMapper().getQuestion().getType();
		if(type != null && type.equals(QuestionType.CODING.getType())){
			Integer passThreshold = getQuestionMapper().getQuestion().getPassPercentforCodingQAsPerWeightedScore();
	...
	public Float getCodingScore() {
		...
		Integer weightPositive = getQuestionMapper().getQuestion().getWeightInputPositive() == null?1:getQuestionMapper().getQuestion().getWeightInputPositive();
		Integer weightNegative = getQuestionMapper().getQuestion().getWeightInputNegative() == null?1:getQuestionMapper().getQuestion().getWeightInputNegative();
		Integer weightExtremePositive = getQuestionMapper().getQuestion().getWeightExtremePositiveValue() == null?1:getQuestionMapper().getQuestion().getWeightExtremePositiveValue();
		Integer weightExtremeMinimal = getQuestionMapper().getQuestion().getWeightExtremeMinimalValue() == null ? 1: getQuestionMapper().getQuestion().getWeightExtremeMinimalValue();
		Integer weightInvalidData = getQuestionMapper().getQuestion().getWeightInvalidDataValue() == null ? 1: getQuestionMapper().getQuestion().getWeightInvalidDataValue();
