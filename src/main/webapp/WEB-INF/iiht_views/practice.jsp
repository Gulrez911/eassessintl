...
<body>
  ...
  <form name="testcodingForm" id="testcodingForm"  method="post" modelAttribute="program" action="savePracticeCode">
		...
		<div class="w3-sidebar w3-light-grey w3-bar-block" style="width: 16%;background-color: #a2b9bc;position:absolute !important;">
			
			<p style="font-size:15px;color:grey;">Your Saved Programs - ${programs.size()}</p>
			 <c:forEach  items="${programs}" var="program" varStatus="loop">   
			 <a href="yakshacode?companyId=${program.companyId}&user=${program.user}&testid=${program.id}" class="w3-bar-item w3-button" style="${program.style};font-size: 13px !important">${program.name}</a>
			 </c:forEach>
		...
		<form:textarea id="editor" path="program.code" wrap="physical" style="width:90%"/>
		<form:hidden path="program.code" id="codeOfEditor" wrap="physical"/>
			<br> <br>
			
			<form:input path="program.input"  style="width:90%" id="program.input" placeholder="Enter input"/>
			<br> <br>
			<form:textarea  style="overflow-y: scroll;rows:16;width:90%;height:100px;" path="program.output"   id="program.output" disabled="true"/>
			<br> <br>
			<a href="javascript:runCode();" align="center" style="background-color: blue;color: #fff;padding: 10px 20px;font-size:13px">Compile and RUN</a>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			
			<a href="javascript:saveCode();" align="center" style="background-color: blue;color: #fff;padding: 10px 20px;font-size:13px">Save Code</a> 
...
<script>
	   ...
	   //editor
	   <%
	   TestProgram program = (TestProgram) request.getAttribute("program");
	   System.out.println("program "+program.getLang());
	  // String cd = StringEscapeUtils.escapeEcmaScript(program.getCode());
	  String cd = program.getCode().replace("'", "\\'");
	  // cd = cd.replaceAll("\r", "");
	//	String rep = "\\\\n";
	//	 String rept = "\\\\t";
	//	cd = cd.replaceAll("\n", rep);
	//	cd = cd.replaceAll("\t", rept);
	   %>
	   var code = '<%= cd %>';
